...
MinIO S3 là tầng lưu trữ, Spark xử lý dữ liệu, Trino/Hive làm catalog, dbt-trino biến đổi dữ liệu, Apache Superset hiển thị dashboard, Airflow điều phối luồng.
...
# 2. Chạy toàn bộ pipeline (MinIO setup → Spark batch → Trino → dbt → CDC bridge → verify)
make k8s-pipeline
